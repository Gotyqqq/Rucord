...
### 7.1. Шумоподавление микрофона (RNNoise)

В приложении добавлено шумоподавление по аналогии с Discord (Krisp): бесплатная библиотека **RNNoise** (WASM) обрабатывает звук микрофона в браузере.

**Что сделать тебе:**

1. **Локально / после деплоя** — ничего дополнительно настраивать не нужно. Worklet попадает в сборку фронта (`dist/assets/noise-suppressor-worklet.js`).
2. **Как включить:** открой **Настройки** (иконка шестерёнки) → вкладка **«Голос и видео»** → включи переключатель **«Шумоподавление (RNNoise)»**.
3. **Когда применится:** настройка сохраняется в браузере. Чтобы она заработала, нужно **один раз выйти из голосового канала и зайти снова** (или зайти в канал после включения опции).
4. **Если не работает:** в консоли браузера (F12) может появиться предупреждение `RNNoise worklet failed...` — тогда будет использоваться только встроенное шумоподавление браузера. Убедись, что в продакшене по адресу `https://твой-домен/assets/noise-suppressor-worklet.js` отдаётся файл (проверь в Nginx, что папка `dist` и подпапка `assets` доступны).
